...
def test_cmd_run_experiment_canonical_prints_plot_notice(
    ...
    monkeypatch.setitem(
        __import__("sys").modules,
        "bnsyn.experiments.declarative",
        SimpleNamespace(run_from_yaml=lambda *_: None, run_canonical_live_bundle=lambda *_: {"artifact_dir": "x"}),
    )
...
def test_cmd_run_experiment_canonical_uses_default_output_dir(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_bundle(_: str, out: str | None) -> dict[str, str]:
        seen.append(out)
    ...
    monkeypatch.setitem(
        __import__("sys").modules,
        "bnsyn.experiments.declarative",
        SimpleNamespace(run_from_yaml=lambda *_: None, run_canonical_live_bundle=fake_bundle),
    )
...
def test_cmd_run_experiment_canonical_returns_1_on_bundle_failure(
    ...
    def raise_bundle(_: str, __: str | None) -> dict[str, str]:
        raise RuntimeError("bundle failed")

    monkeypatch.setitem(
        __import__("sys").modules,
        "bnsyn.experiments.declarative",
        SimpleNamespace(run_from_yaml=lambda *_: None, run_canonical_live_bundle=raise_bundle),
    )
