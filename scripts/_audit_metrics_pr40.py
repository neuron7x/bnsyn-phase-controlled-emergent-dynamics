...
def audit_metrics(results_dir: Path) -> dict:
    ...
    if fixed_high_w_total_var > 0:
        reduction_w_total = ((fixed_high_w_total_var - cooling_w_total_var) / fixed_high_w_total_var) * 100
    else:
        reduction_w_total = 0.0
    
    if fixed_high_w_cons_var > 0:
        reduction_w_cons = ((fixed_high_w_cons_var - cooling_w_cons_var) / fixed_high_w_cons_var) * 100
    else:
...
def main():
    ...
    print(f"Ratio (fixed_high / cooling):        {audit['summary']['ratio_w_cons']:.2f}x" if audit['summary']['ratio_w_cons'] != float('inf') else "Ratio: ∞ (cooling var = 0)")
    ...
    for condition in ["cooling_geometric", "fixed_high", "fixed_low", "random_T"]:
        ...
        print(f"{condition:20} {status}  w_total_diff={w_total_diff:.2e}  w_cons_diff={w_cons_diff:.2e}")
