...
3. Current HEAD run query evidence is exported in `artifacts/audit/runs_for_head.json`; `total_count=0` for local HEAD SHA in this runtime.
...
3. CI green status for the current PR head is not provable from immutable run URL evidence in this runtime because no runs exist for local HEAD SHA.

## GAP STATUS

See `GAP_TABLE.md`.
- Closed: GAP-001, GAP-002, GAP-003, GAP-004.
- Still blocking without external action: GAP-005.

## FINAL STATUS (fail-closed)

- `CI_EXECUTABILITY_STATUS = NEEDS_EVIDENCE` (current HEAD has no CI run URLs in `runs_for_head.json`).
- `BATTLE_USAGE_STATUS = FORMALIZED_NON_USAGE` (explicit non-usage declaration + CI enforcement).
- `READYNESS_PERCENT = 75` using strict fail-closed scoring:
  - Start 100
  - Missing current-HEAD CI proof: -25
