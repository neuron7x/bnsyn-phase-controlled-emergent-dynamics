...
- **Production deterministic-mode guardrail**
  - `prod` forbids deterministic mode by default to prevent expiry-freeze/downgrade behavior.
  - Emergency override is explicit: `BNSYN_ALLOW_DETERMINISTIC_IN_PROD=1`.
...
- **CSRF posture for `/logout`**
  ...
  - In `dev`/`prod`, if `Origin` or `Referer` header is present, scheme+host must match `allowed_origins`; cross-site values return `403`.
...
## Consequences
- Production defaults become fail-closed and time-correct, including explicit deterministic-mode downgrade prevention in `prod`.
