...
- docs are validated structurally (Markdown sections + list items), not by loose substring search alone
- CI keeps lint/type/security gates blocking and now exercises `tests/test_release_readiness.py` directly in the contracts lane for faster regression feedback
...
- Malformed governance inputs now produce deterministic blocking results instead of bubbling exceptions, which is an intentional fail-closed hardening.
