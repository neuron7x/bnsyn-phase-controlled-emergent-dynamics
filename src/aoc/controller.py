...
class AOCController:
    def __init__(
        ...
        self.telemetry = TelemetryLogger.for_artifact_dir(
            run_dir,
            run_id=new_run_id("aoc"),
            default_stage="aoc_controller",
        )

    def run(self) -> dict[str, object]:
        run_started = self.telemetry.time_stage("aoc_controller")
        try:
            ...
            self.telemetry.emit(
                ...
                stage="aoc_controller",
                status=str(verdict["status"]),
                failure_reason=None if verdict["status"] == "PASS" else str(verdict["stop_reason"]),
                verdict=verdict,
            )
            ...
            self.telemetry.finish_stage("aoc_controller", run_started, status=str(verdict["status"]).lower())
            return verdict
        except Exception as exc:
            self.telemetry.fail_stage(
                "aoc_controller",
                run_started,
