...
class AdExNeuron:
    ...
    @classmethod
    def init(cls, n: int, params: AdExParams | None = None, *, V0: float | None = None) -> "AdExNeuron":
    ...
    def step(self, I: np.ndarray, dt: float, t: float) -> Tuple[np.ndarray, np.ndarray]:
        """Advance state by one Euler step.

        Args:
            I: input current (Amps), shape (n,)
            dt: timestep (seconds)
        ...
        if I.shape != self.V.shape:
            raise ValueError(f"I shape {I.shape} must match V shape {self.V.shape}")
        ...
        dV = (-(p.gL * (V_eff - p.EL)) + exp_term - self.w + I) / p.C
