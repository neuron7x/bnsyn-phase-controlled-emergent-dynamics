...
def run_emergence_to_disk(
    ...
    injected_current = (
        np.full(N, external_current, dtype=np.float64)
        if abs(external_current) > 1e-9
        else None
    )
    ...
    target_path = output_dir / f"run_{seed}_Iext_{int(external_current)}pA.npz"
    ...
    np.savez(
        ...
        N=np.int64(N),
        seed=np.int64(seed),
        external_current_pA=np.float64(external_current),
    )
