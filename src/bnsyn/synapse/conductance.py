...
class ConductanceState:
    ...
    g_gabaa_nS: np.ndarray


def nmda_mg_block(V_mV: np.ndarray, mg_mM: float) -> np.ndarray:
...
class ConductanceSynapses:
    ...
    def __init__(self, N: int, params: SynapseParams, dt_ms: float):
        if N <= 0:
        ...
        self._buf = np.zeros((delay_steps, N), dtype=float)
        self._buf_idx = 0

        self.state = ConductanceState(
            g_ampa_nS=np.zeros(N, dtype=float),
            g_nmda_nS=np.zeros(N, dtype=float),
            g_gabaa_nS=np.zeros(N, dtype=float),
        )
    ...
    def queue_events(self, incoming: np.ndarray) -> None:
        ...
        self._buf[self._buf_idx, :] = incoming
